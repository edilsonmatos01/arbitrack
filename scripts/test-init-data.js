...
async function testInitData() {
  try {
    ...
    console.log(`   Dados históricos: ${data.historicalData?.length || 0}`);
    console.log(`   Comparação de preços: ${data.priceComparison?.length || 0}`);
    
    console.log('\n📈 Spreads máximos encontrados:');
    if (data.maxSpreads) {
      data.maxSpreads.forEach(spread => {
        if (spread.maxSpread > 0) {
          console.log(`   ${spread.symbol}: ${spread.maxSpread.toFixed(2)}% (${spread.count} registros)`);
        }
      });
    }
