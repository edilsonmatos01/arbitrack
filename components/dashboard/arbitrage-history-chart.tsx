...
export default function ArbitrageHistoryChart({ operations: externalOperations, onDataUpdate }: ArbitrageHistoryChartProps) {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length > 0) {
      ...
      return (
        <div className="bg-gray-800 border border-gray-700 p-3 rounded-md shadow-lg glow-on-hover">
          ...
          <p className="text-custom-cyan text-sm">{`Lucro do dia: $${data.totalProfit}`}</p>
        </div>
      );
    }
    return null;
        ...
        <div className="flex space-x-1">
          {filterButtons.map((period) => (
            <button 
              ...
              className={`btn-enhanced px-3 py-1 text-xs rounded-md focus:outline-none transition-colors glow-on-hover
