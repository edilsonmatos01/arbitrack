...
function SpreadCard({ spread }: { spread: SpreadInfo }) {
  const getSpreadColor = (spreadValue: number) => {
    if (spreadValue >= 1.0) return 'text-accent-orange';
    if (spreadValue >= 0.5) return 'text-accent-amber';
  ...
  return (
            <div className="card-enhanced card-neon-intense p-4 border border-gray-700">
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm">
          ...
          <span className="text-white font-medium">{spread.buyExchange}</span>
        </div>
        <div className="flex justify-between text-sm">
