...
function AlertItem({ alert, onDismiss }: AlertItemProps) {
  const getAlertConfig = (type: Alert['type']) => {
    switch (type) {
      ...
      default:
        return {
          icon: <Info className="h-5 w-5 text-custom-cyan" />,
          bgColor: 'bg-custom-cyan/20',
          borderColor: 'border-custom-cyan/30',
          textColor: 'text-custom-cyan'
        };
    }
  };
      ...
      <div className="flex items-start gap-3">
        ...
        {alert.dismissible && (
          <button
            ...
            className="btn-enhanced flex-shrink-0 text-gray-400 hover:text-white transition-colors p-1 hover:bg-gray-800 rounded glow-on-hover"
          >
            <X className="h-4 w-4" />
          </button>
        )}
