...
export default function Sidebar({ user, navItems }: SidebarProps) {
  ...
  return (
    <aside className="w-72 card-enhanced p-6 flex flex-col text-gray-300 min-h-screen">
          ...
          {navItems.map((item) => {
            ...
            return (
              <li key={item.title}>
                <Link
                  href={item.href}
                  className={`btn-enhanced flex items-center p-3 rounded-lg hover:bg-gray-700 hover:text-white transition-colors group glow-on-hover ${
