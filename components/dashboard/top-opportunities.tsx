...
function OpportunityCard({ opportunity, rank }: OpportunityCardProps) {
  const getRankColor = (rank: number) => {
    switch (rank) {
      case 1: return 'text-accent-orange bg-accent-orange/10 border-accent-orange/20';
      case 2: return 'text-gray-300 bg-gray-300/10 border-gray-300/20';
      case 3: return 'text-accent-amber bg-accent-amber/10 border-accent-amber/20';
  ...
  return (
    <div className="card-enhanced card-neon-intense p-4 border border-gray-700 hover:border-gray-600 transition-all">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-xs text-gray-400">
              <Clock className="h-3 w-3 text-custom-cyan" />
              <span>{formatTime(opportunity.timestamp)}</span>
              {opportunity.isLive && (
                <div className="flex items-center gap-1 text-custom-cyan">
                  <div className="w-1.5 h-1.5 bg-custom-cyan rounded-full animate-pulse"></div>
                  <span>LIVE</span>
                </div>
              )}
            </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg glow-on-hover">
          <div className="text-center flex-1">
            <div className="text-sm text-gray-400 mb-1">Comprar em</div>
            <div className="font-semibold text-white">{opportunity.buyExchange}</div>
            <div className="text-xs text-gray-500">${opportunity.buyPrice.toFixed(4)}</div>
          </div>
          
          <div className="px-3">
            <ArrowRight className="h-5 w-5 text-custom-cyan" />
          </div>
