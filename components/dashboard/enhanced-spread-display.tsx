...
function SpreadCard({ data, isLive = false }: SpreadCardProps) {
  const getSpreadColor = (spread: number) => {
    if (spread >= 1.0) return 'text-accent-orange';
    if (spread >= 0.5) return 'text-accent-amber';
  ...
  return (
    <div className="card-enhanced card-neon-intense p-4 border border-gray-700 hover:border-gray-600 transition-colors">
        ...
        <div className="flex items-center gap-2">
          ...
          {isLive && (
            <div className="flex items-center gap-1">
              ...
              <Zap className="h-3 w-3 text-custom-cyan" />
            </div>
          )}
        </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className={`font-semibold ${data.profit > 0 ? 'text-white' : 'text-red-400'}`}>
