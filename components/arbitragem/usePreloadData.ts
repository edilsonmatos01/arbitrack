...
export function usePreloadData() {
  ...
  useEffect(() => {
    console.log('[PreloadData] useEffect executado, globalCache:', !!globalCache, 'isLoading:', isLoading);
    
    // Se já temos cache, usar imediatamente
    if (globalCache && !isInitialized) {
      console.log('[PreloadData] Usando cache existente IMEDIATAMENTE');
      setData(globalCache);
      setLocalLoading(false);
      setIsInitialized(true);
      return;
    ...
    // Se não temos cache e não está carregando, carregar IMEDIATAMENTE
    if (!globalCache && !isLoading && !isInitialized) {
      console.log('[PreloadData] Iniciando pré-carregamento ULTRA-RÁPIDO...');
      // Usar setTimeout para garantir que seja executado após a renderização
      setTimeout(() => {
        preloadAllData();
      }, 0);
    }
  }, [isInitialized, preloadAllData]);

  // Funções utilitárias
  const getSpreadData = useCallback((symbol: string): SpreadData | null => {
    if (!data?.spreads) return null;
