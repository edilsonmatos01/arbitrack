...
async function startFeeds() {
    ...
    try {
        ...
        // Aguarda um pouco para as conexões se estabelecerem
        setTimeout(async () => {
            try {
                // Busca pares disponíveis da MEXC
                const mexcPairs = await mexc.getTradablePairs();
                console.log(`[FEEDS] MEXC retornou ${mexcPairs.length} pares`);
                
                // Adiciona os pares principais primeiro
                const priorityPairs = ['BTC_USDT', 'ETH_USDT', 'SOL_USDT', 'XRP_USDT', 'BNB_USDT', 'WHITE_USDT'];
                const priorityMexc = priorityPairs.filter(symbol => mexcPairs.includes(symbol));
                
                // Adiciona os pares à MEXC
                await mexc.addSymbols(priorityMexc);
                
                // Define os target pairs para arbitragem
                targetPairs = priorityMexc;
                
                console.log(`[FEEDS] Target pairs definidos: ${targetPairs.length} pares`);
                console.log(`[FEEDS] Pares principais: ${priorityMexc.join(', ')}`);
                console.log(`[FEEDS] WHITE_USDT incluído: ${targetPairs.includes('WHITE_USDT')}`);
            } catch (error) {
                console.error('[FEEDS] Erro ao configurar pares:', error);
            }
        }, 3000);
