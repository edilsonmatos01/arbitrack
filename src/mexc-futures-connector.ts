...
export class MexcFuturesConnector {
    ...
    subscribe(pairs: string[]) {
        ...
        try {
            ...
            formattedPairs.forEach((symbol, index) => {
                ...
                if (index % 10 === 0 || index < 5 || index >= formattedPairs.length - 5) {
                    console.log(`[${this.identifier}] Subscrição ${index + 1}/${formattedPairs.length}: ${symbol}`);
                }
                
                // Pequeno delay entre subscrições
                setTimeout(() => {}, 50);
            });
            
            pairs.forEach(symbol => this.subscribedSymbols.add(symbol));
            console.log(`[${this.identifier}] ✅ Todas as ${pairs.length} subscrições enviadas!`);
        } catch (error) {
